...
getDataForOutcomeTable <- function(plot_specs, ind, estimates_tab, order) {
  ...
  for (kk in 1:length(index)) {
    if (kk == length(index)) {
      data_table <- data_table %>%
        union_all(rbind(num_ind[kk,],
                        table[index[kk]:nrow(table),] %>% mutate ("Staggered cohort" = NA)))
    } else {
      data_table <- data_table %>%
        union_all(rbind(num_ind[kk,],
                        table[index[kk]:(index[kk+1]-1),] %>% mutate ("Staggered cohort" = NA)))
    }
  }
