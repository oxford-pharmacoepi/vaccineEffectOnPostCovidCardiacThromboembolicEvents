...
start.date1 <- as.Date("2021-02-20") # Start date of the study 1 (first vaccination included)
end.date1 <- as.Date("2021-03-22") # End date of the study 1 (last vaccination included)
start.date2 <- as.Date("2021-03-24") # Start date of the study 2 (first vaccination included)
end.date2 <- as.Date("2021-05-04") # End date of the study 2 (last vaccination included)
start.date3 <- as.Date("2021-05-05") # Start date of the study 3 (first vaccination included)
end.date3 <- as.Date("2021-06-16") # End date of the study 3 (last vaccination included)
start.date4 <- as.Date("2021-06-17") # Start date of the study 4 (first vaccination included)
end.date4 <- as.Date("2021-08-01") # End date of the study 4 (last vaccination included)
followback <- 180 # Minimum follow back needed
follow.date <- start.date1 - followback

# Cohort definition ID's

covid_id <- covidCohortSet %>%
  filter(cohort_name == "covid19") %>%
  pull("cohort_definition_id")

any_vaccine_id <- vaccinatedCohortSet %>%
  filter(cohort_name == "any_vaccine") %>%
  pull("cohort_definition_id")

high_risk_id <- databaseSpecificCohortSet %>%
...
high_risk <- cdm[[databaseSpecificCohortName]] %>%
  dplyr::inner_join(tibble::tibble(cohort_definition_id = .env$high_risk_id), copy = TRUE) %>%
  dplyr::select("person_id" = "subject_id", "high_risk_date" = "cohort_start_date") %>%
  dplyr::distinct() %>%
  dplyr::compute()
...
individuals <- cdm[["person"]] %>%
  dplyr::select("person_id", "year_of_birth", "location_id")
...
individuals <- individuals %>%
  dplyr::inner_join(cdm$location %>%
                      filter(location_source_value != "NO ESPECIFICAT"), 
                    by = "location_id"
  ) %>%
  dplyr::compute()
...
attrition <- rbind(
  ...
  dplyr::tibble(
    number_observations = number_observations,
    reason = "Location ID not recorded",
